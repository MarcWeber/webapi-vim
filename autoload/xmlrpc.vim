...
function! s:to_value(content)
  if type(a:content) == 4
    if has_key(a:content, 'bits')
      ...
      call add(struct.childs, member)
      let name = xml#createElement("name")
      call add(member.childs, name)
      call name.value("name")
      let value = xml#createElement("value")
      call add(member.childs, value)
      call add(value.childs, s:to_value(a:content["name"]))
      ...
      for key in keys(a:content)
        ...
        call add(member.childs, name)
        let value = xml#createElement("value")
        call add(value.childs, s:to_value(a:content[key]))
        call add(member.childs, value)
        call add(struct.childs, member)
      endfor
      return struct
  ...
  elseif type(a:content) == 3
    ...
    for item in a:content
      let value = xml#createElement("value")
      call add(value.childs, s:to_value(item))
      call add(data.childs, value)
    endfor
    call add(array.childs, data)
    return array
...
function! xmlrpc#call(uri, func, args)
  ...
  call add(methodCall.childs, methodName)
  let params = xml#createElement("params")
  for arg in a:args
    let param = xml#createElement("param")
    let value = xml#createElement("value")
    call value.value(s:to_value(arg))
    call add(param.childs, value)
    call add(params.childs, param)
    unlet arg
  endfor
  call add(methodCall.childs, params)
